...
if [ "$# != "2" ]; then
	...
	function ProgressBar {
		let _progress = ($r{(file_Count} * 100)/100
	...
	find /opt/breach-parse/BreachCompilation/data -type f -print0 | whileread -d $'\0' file 

	do 
		grep -a -E "$1" "$file" >> $master
		((++file_Count))
		ProgressBar ${nummber} ${total_Files}

	done

fi #this is here to end if-statement. 
